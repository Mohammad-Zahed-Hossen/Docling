...
from docling_api.converter import ConversionArtifacts
...
def test_health(client: TestClient) -> None:
    ...
    assert response.json() == {"status": "ok", "service": "docling-local-engine"}


def test_rejects_non_pdf(client: TestClient) -> None:
    response = client.post("/api/convert", files={"file": ("notes.txt", b"hello", "text/plain")})
...
def test_converter_failure_is_standardized(tmp_path: Path) -> None:
    ...
    assert response.json() == {
        "error": {"code": "CONVERSION_FAILED", "message": "Docling could not convert this PDF."}
    }
...
def test_success_and_downloads(tmp_path: Path) -> None:
    class SuccessfulEngine:
        ...
        def convert(self, _input: Path, result_dir: Path, stem: str) -> ConversionArtifacts:
            ...
            return ConversionArtifacts("# Test", md_path, zip_path, 1, 0.1, 0, 0)
