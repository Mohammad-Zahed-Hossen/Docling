...
export function SettingsPanel({ open, apiUrl, onApiUrlChange, onReset, onClose, onRetry, options, onOptionsChange, theme, onThemeChange, inputType }: Props) {
  ...
  return (
    ...
    <aside ref={panelRef} className="settings-panel" role="dialog" aria-modal="true" aria-labelledby="settings-title">
      ...
      {inputType === "file" && <fieldset><legend>Conversion</legend>
      <div className="quick-settings">
      ...
      <div className="quick-settings">
        {inputType === "file" && <label>CPU usage<select value={options.cpu} onChange={(e) => onOptionsChange({...options, cpu: e.target.value as ConversionOptions["cpu"]})}><option value="balanced">Balanced</option><option value="maximum">Maximum</option></select></label>}
